...
async def convert_action_item_to_task(
    ...
    try:
        # Import the converter lazily to avoid import-time failures if Composio is missing
        try:
            from automations.meeting_task_converter import MeetingTaskConverter
        except Exception as e:
            # Provide clear error to caller that automation integration isn't available
            raise HTTPException(
                status_code=500,
