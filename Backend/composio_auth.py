...
def initiate_trello_connection(api_key: str, redirect_url: Optional[str] = None) -> Dict[str, Any]:
    ...
    Returns a dict with keys: redirect_url, connection_id (if immediate), status
    """
    try:
        client = _client_for_key(api_key)
        user_id = _make_user_id(api_key)

        # Authorize the user for the Trello toolkit (Composio will create a managed auth config if missing)
        # This returns a connection request object with redirectUrl and wait_for_connection capability.
        conn_request = client.toolkits.authorize(user_id=user_id, toolkit="trello")

        # conn_request should have at least a redirectUrl and an id (connection request id)
        redirect = getattr(conn_request, "redirectUrl", None) or conn_request.get("redirectUrl") if isinstance(conn_request, dict) else None
        connection_id = getattr(conn_request, "id", None) or conn_request.get("id") if isinstance(conn_request, dict) else None
        status = getattr(conn_request, "status", None) or conn_request.get("status") if isinstance(conn_request, dict) else None
...
def check_trello_connection(api_key: str) -> Dict[str, Any]:
    ...
    Returns { is_connected: bool, connected_account (if any), details/error }
    """
    try:
        client = _client_for_key(api_key)
        user_id = _make_user_id(api_key)

        # List connected accounts for this user filtered for Trello (toolkit slug 'trello')
        # Use list with user_ids and optionally toolkit filter
        accounts = client.connected_accounts.list(user_ids=[user_id], toolkits=["trello"])

        if not accounts:
            return {"is_connected": False, "connected_account": None, "status": None, "error": "No connected accounts found"}

        # Prefer ACTIVE account
        active = next((a for a in accounts if (a.get("status") == "ACTIVE" if isinstance(a, dict) else getattr(a, "status", None) == "ACTIVE")), None)
        chosen = active or accounts[0]

        # normalize
        if isinstance(chosen, dict):
            conn_id = chosen.get("id")
            status = chosen.get("status")
            app_unique_id = chosen.get("appUniqueId") if "appUniqueId" in chosen else chosen.get("app_unique_id")
        else:
            conn_id = getattr(chosen, "id", None)
            status = getattr(chosen, "status", None)
            app_unique_id = getattr(chosen, "appUniqueId", None) or getattr(chosen, "app_unique_id", None)
...
def get_trello_boards(api_key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            # fallback signature (some SDK versions call with (action, params, user_id=...))
            result = client.tools.execute("TRELLO_GET_MEMBERS_BOARDS_BY_ID_MEMBER", {"id": "me"}, user_id=user_id)
...
def get_trello_cards(api_key: str, board_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        action_slug = "TRELLO_GET_BOARDS_CARDS_BY_ID_BOARD"
        try:
            result = client.tools.execute(action=action_slug, params={"idBoard": board_id}, user_id=user_id)
        except TypeError:
            result = client.tools.execute(action_slug, {"idBoard": board_id}, user_id=user_id)
